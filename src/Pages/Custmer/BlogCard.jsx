...
const BlogCard = ({ thumbnail, createdAt, readTime, category, heading, blogDescription, loading ,_id}) => {
 ...
 <div className={`bg-white rounded-3xl shadow-md overflow-hidden ${loading ? 'shimmer' : ''}`}>
  ...
  <div className="p-4">
    ...
    {loading ? (
      ...
      <EllipsizedParagraph text={blogDescription}/>
      // <p className="text-gray-600 mb-4">{reportDescription}</p>
    )}
    {!loading &&    
      <Link  
        to={`/ArticleDetail/${encodeURIComponent(heading)}/${encodeURIComponent(createdAt)}/${encodeURIComponent(category)}/${encodeURIComponent(blogDescription)}/${encodeURIComponent(thumbnail)}/${encodeURIComponent(_id)}`}
